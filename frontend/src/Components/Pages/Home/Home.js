...
import Service from "../ServiceCenter/Service";
...
const Home = ({ id }) => {
  ...
  useEffect(() => {
    const getuserData = async () => {
      await axios
        .get(`http://localhost:8000/hasUploadlifecertificate/${id._id}`)
  ...
  useEffect(() => {
    if (status.message == "NOTUPLOAD") {
      alert("you have not uploaded lifecertificate yet");
    }
  }, [status]);
  ...
  const [servicecenterViewpage, setservicecenterViewpage] = useState(false);

  const uploadfromview = () => {
  ...
  const servicecenterview = () => {
    setservicecenterViewpage(!servicecenterViewpage);
    sethome(!home);
  };

  return (
    <>
      {id ? <Navbar id={id} /> : <Navbar />}
      {home ? (
        <>
          <div className="container">
            <div className=" py-md-32 position-relative mt-7">
              {/*  py-md-32  */}
                ...
                <div className="row align-items-center">
                      ...
                      <div className="svg-fluid position-relative overlap-10">
                        ...
                        >
                          <path
                            ...
                            transform="translate(-114.4741 -135.80928)"
                            fill="#cacaca"
                          />
                          <path
                  ...
                  <div className="col-lg-6 order-md-0">
                    ...
                    <p className="lead mb-5">
                      700.39 Lakh Jeevan Pramaan Already submitted Digital Life
                      Certificates since 2014
                    </p>

                    <div className="mx-n2">
                      {id ? (
                        <button
                          onClick={uploadfromview}
      ...
      {uploadformView ? (
        <>
          {id ? (
            <Upload
              uploadfromview={uploadfromview}
              id={id}
              userdata={userdata}
            />
          ) : (
            <Upload uploadfromview={uploadfromview} />
          )}
        </>
      ) : (
        <></>
      ...
      {servicecenterViewpage ? (
        <>
          {" "}
          <Service uploadfromview={servicecenterview} />
        </>
      ) : (
        <></>
      )}
